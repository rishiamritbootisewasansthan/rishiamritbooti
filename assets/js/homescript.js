...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function typeText() {
        if (currentIndex < textLength) {
            textElement.innerText = originalText.substr(0, currentIndex + 1);
            currentIndex++;
            setTimeout(typeText, 60); // Adjust the delay as needed for the desired typing speed
        } else {
            currentIndex = 0; // Reset the index to start over
            setTimeout(typeText, 3000); // Delay before starting over (1 second in this example)
        }
    }
